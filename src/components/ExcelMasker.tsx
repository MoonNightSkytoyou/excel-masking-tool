...
import { CellModel, MaskingConfig, EntityType, Operator, SessionContext } from '../types';
...
import { TRANSLATIONS, getEntityLabel, getOperatorLabel, Locale } from '../i18n/translations';
import { ENTITY_COLORS } from '../config/entity-colors';
                      ...
                      <div className="space-y-2.5 max-h-64 overflow-y-auto pr-1">
                        {(Object.keys(DEFAULT_GLOBAL_OPERATORS) as EntityType[]).map((type) => (
                          <div key={type} className="flex items-center justify-between gap-3 text-xs bg-gray-50 p-2.5 rounded-lg border border-gray-100">
                            <span className="font-medium text-gray-700">{getEntityLabel(t, type)}</span>
                            <select
                            ...
                            >
                              {(Object.keys(t.opLabels) as Operator[]).map((op) => (
                                <option key={op} value={op}>{getOperatorLabel(t, op)}</option>
                              ))}
                            </select>
                          </div>
                        ))}
                                  ...
                                  <td className="py-3.5 px-4">
                                    {col.detectedType ? (
                                      <span className={`inline-flex px-2 py-1 rounded text-xs font-semibold border ${ENTITY_COLORS[col.detectedType].color} ${ENTITY_COLORS[col.detectedType].bg}`}>
                                        {getEntityLabel(t, col.detectedType)}
                                      </span>
                                    ) : (
                                  ...
                                  <td className="py-3.5 px-4">
                                    ...
                                    >
                                      ...
                                      {(Object.keys(DEFAULT_GLOBAL_OPERATORS) as EntityType[]).map((typeKey) => (
                                        <option key={typeKey} value={typeKey}>{getEntityLabel(t, typeKey)}</option>
                                      ))}
                                    </select>
                                  </td>
                                  <td className="py-3.5 px-4">
                                    <div className="flex flex-col gap-1.5">
                                      ...
                                      >
                                        {(Object.keys(t.opLabels) as Operator[]).map((op) => (
                                          <option key={op} value={op} disabled={override.selectedType === 'NONE' && (op === 'PSEUDO' || op === 'JITTER')}>{getOperatorLabel(t, op)}</option>
                                        ))}
                                      </select>
